// Pozicování dropdownu přesně pod toggle (desktop: levá hrana = čárky; mobil: na padding)
(async function () {
  ...
  ['resize','scroll'].forEach(ev=> window.addEventListener(ev, ()=>{ if(drawer.classList.contains('open')) positionDrawer(); }));
  ...
  drawer.querySelectorAll('a').forEach(a=>{
    ...
    ['touchend','touchcancel'].forEach(ev=> a.addEventListener(ev, ()=> a.classList.remove('is-hover'), {passive:true}));
    a.addEventListener('click', closeMenu);
  });

  mqMobile.addEventListener?.('change', ()=>{ if (drawer.classList.contains('open')) positionDrawer(); });
})();
